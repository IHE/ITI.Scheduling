...
In the case when a Scheduling Client chooses to add multiple patient.identifiers to the Find Existing Appointments Query transaction the Scheduling Server shall evaluate all identifiers to find a matching patient. 

###### _count
A Scheduling Client may add a ```_count``` parameter to the Find Appointment Query request to limit the number of responses. 

###### _include
A Scheduling Client may add a ```_include_``` parameter to request a Scheduling Server to include participation actors (Patient, Practitioner, PractionerRole, RelatedPerson, Device, HealthcareService, or Location) in the response 
